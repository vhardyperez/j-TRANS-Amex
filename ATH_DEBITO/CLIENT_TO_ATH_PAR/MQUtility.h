...
//	RWCString SetB24Header0200Pos(RWCString incoming_header); // No se usa
	...
	RWCString SendMessageToQueueNewATHDebito(RWCString incoming, char *send_queue_input, char *queue_mgr_input);
	...
	RWCString Get_Msg_From_SendQueueNew( char *send_queue,char *queue_mgr);   
	void SendMessageToQueueHeaderVisaThreeParameters(RWCString incoming, char *recv_queue_input, char *queue_mgr_input);

	RWCString SetSqlFieldFrom0420(RWCString incoming , char *msg_type, 
										           RWCString application,RWCString table_msg );
	
	RWCString SendMessageToQueueWithParameters(RWCString incoming, char *send_queue_input, char *queue_mgr_input ,char *recv_queue_input, char *sql_queue_input);
	RWCString SendMessageToQueuePagosWithParameters(RWCString incoming, char *send_queue_input, char *queue_mgr_input ,char *recv_queue_input, char *sql_queue_input);

	RWCString SendMessageToQueueRetornoMsgBtransVarDelimiter1CPar(RWCString incoming, char *send_queue_string, char *queue_mgr_string,
					 char *sql_queue_string , char *recv_queue_string);

	RWCString SetIso8583B24Msg0210FromIso8583B24Msg0200ForScript(RWCString incoming , char *msg_type, 
										          int debug, RWCString application,RWCString answer);

	RWCString SetSqlFieldFrom0420Gm(RWCString incoming , char *msg_type, 
										           RWCString application );

	void MQUtility::SendMessageToQueueSqlFromSpecialReturn(RWCString incoming,  
	             char *application_string,char *header_string, char *sql_queue_string , char *queue_mgr_string, char *return_code);

    RWCString GetGeneric0210FromVar1C( RWCString incoming, RWCString error_value);
	RWCString SendMessageToQueueNewVBV1CPar(RWCString incoming, char *send_queue_string, char *queue_mgr_string,
					 char *sql_queue_string , char *recv_queue_string);
	RWCString MQUtility::VerifyIso8583FieldFromB24Format(RWCString incoming , char *msg_type, 
										          int debug, RWCString application);
	void Proccess_Message_PostilionNoField125( RWCString incoming_msg, char *recv_queue, char *queue_mgr);

	RWCString SetBtransFormat1CFromB24Format(RWCString incoming , char *msg_type, 
										          int debug, RWCString application);

	RWCString SendMessageToQueueRetornoMsgBtransVarDelimiter1CParNoField49(RWCString incoming, char *send_queue_string, char *queue_mgr_string,
					 char *sql_queue_string , char *recv_queue_string);
	
	RWCString Retorno_210_B24_1C_NO_Field_49(RWCString incoming , char *msg_type,int debug);
	RWCString MQUtility::GetIso8583FieldB24(RWCString incoming, int debug, RWCString application,int input_field);


	RWCString MQUtility::SendMessageToQueueNewOrange1C(RWCString incoming, char *send_queue_input, char *queue_mgr_input);
	RWCString MQUtility::Retorno_Msg_Iso8583_Field1251C(RWCString incoming , char *msg_type, 
										          int debug, RWCString application);


	RWCString MQUtility::SendMessageToQueueNewOrange1C(RWCString incoming, 
	          char *send_queue_input,char *recv_queue_input,char *sql_queue_input, char *queue_mgr_input);

	/*Se usa para crear 0200 y 0210 para enviar al log PCI complain*/
	RWCString MQUtility::SetIso8583B24MsgForLog(RWCString incoming , char *msg_type, 
										          int debug, RWCString application);

	RWCString MQUtility::SetIso8583B24Msg0210FromIso8583B24Msg0200ForScript2(RWCString incoming , char *msg_type, 
										          int debug, RWCString application,RWCString answer);

	RWCString MQUtility::SetIso8583CapitaGFromB24FormatWithParameters1C(RWCString incoming , char *msg_type, 
										          int debug, RWCString application,RWCString field_19_rw , RWCString field_24_rw,
												  RWCString field_61_rw, RWCString field_62_rw, RWCString field_127_rw);
	void MQUtility::SendMessageToQueueNoTimeoutNew(RWCString incoming, char *send_queue_input, char *queue_mgr_input);

	RWCString MQUtility::GetIso8583B24MsgWithControlField52Fiel18(RWCString incoming, int debug, RWCString application);

	RWCString MQUtility::SendMessageToQueueDomiciliacion(RWCString incoming, char *recv_queue_input ,char *send_queue_input, char *queue_mgr_input,char *sql_queue_input,int time_out);

	RWCString  MQUtility::GetTheIniFileInformationPlataTebca2();
	RWCString MQUtility::SetIso8583B24FromKioscosB24(RWCString incoming , char *msg_type, 
										          int debug, RWCString application);


	RWCString MQUtility::GetIso8583FieldB24ForKioscos(RWCString incoming , char *msg_type, 
										          int debug, RWCString application,int input_field);
	void MQUtility::SendMessageToQueueNewWithKey(RWCString incoming, char *send_queue_string, char *queue_mgr_string);

	
	RWCString MQUtility::GetMesageFromQueueWithkey(RWCString incoming, char *recv_queue_string, char *queue_mgr_string);
	RWCString MQUtility::GetMsgFromSendQueueCapitaG( RWCString key_message,  char *send_queue,char *queue_mgr);

	RWCString MQUtility::SetIso8583B24ForCapitalgB24(RWCString incoming , char *msg_type, 
										          int debug, RWCString application);

	RWCString MQUtility::GetHeaderAtmPosFlag(RWCString incoming );
	...
	RWCString MQUtility::SetIso8583B24Msg0200FromIso8583B24Msg0200(RWCString incoming , char *msg_type, 
										          int debug, RWCString application);
	RWCString MQUtility::SetIso8583B24FromIso8583B24Msg(RWCString incoming , char *msg_type, 
										          int debug, RWCString application);

	RWCString MQUtility::SetIso8583B24Msg0210FromIso8583B24Msg0200ForB24(RWCString incoming , char *msg_type, 
										          int debug, RWCString application,RWCString answer);
	void MQUtility::ShowFieldIso8583B24BothBitMap(RWCString incoming , char *msg_type, 
										          int debug, RWCString application);

	RWCString MQUtility::RetornoAppendMsg(RWCString incoming_msg, char append_char, int append_length);

	RWCString MQUtility::GetBIT11FromOriginal0200(RWCString incoming );


	RWCString MQUtility::SetIso8583B24Msg0200FromIso8583B24Msg0200(RWCString incoming , char *msg_type, 
										          int debug, RWCString application, int field_number , RWCString field_value);

	RWCString MQUtility::SetIso8583B24FromIso8583B24Msg(RWCString incoming , char *msg_type, 
										          int debug, RWCString application, int field_number , RWCString field_value);

	RWCString MQUtility::SetIso8583B24FromIso8583B24MsgResponseMessage(RWCString incoming , char *msg_type, 
										          int debug, RWCString application, int field_number , RWCString field_value);


	void MQUtility::ShowFieldIso8583B24Discover1C(RWCString incoming , char *msg_type, 
										          int debug, RWCString application);

	RWCString MQUtility::SetIso8583B24Msg0210FromIso8583B24Msg02001C(RWCString incoming , char *msg_type, 
										          int debug, RWCString application,RWCString answer);

	RWCString MQUtility::setMapIsoPosTermFromGeneric1C( RWCString incoming, char *application, char *header_msg ,int debug);

	RWCString MQUtility::SetIso8583B24Msg0210FromIso8583B24Msg0200ForScriptB24(RWCString incoming , char *msg_type, 
										          int debug, RWCString application,RWCString answer);
	...
	RWCString MQUtility::SetIso8583B24Msg0210FromIso8583B24Msg0200ForScriptB24ForPostBridge(RWCString incoming , char *msg_type, 
										          int debug, RWCString application,RWCString answer);

	RWCString MQUtility::SetBtransFormatFromB24CommaDelmited(RWCString incoming , char *msg_type, 
										          int debug, RWCString application);

	RWCString MQUtility::GetMsgFromSendQueuePagosWithTimeOut( RWCString key_message,  char *send_queue,char *queue_mgr, int time_out);

	void MQUtility::SendMessageToQueueNewATHDebito0430Vicana(RWCString incoming, char *send_queue_input, char *queue_mgr_input);
    void MQUtility::SendMessageToQueueNew0420FromSAF(RWCString incoming, char *send_queue_input, char *queue_mgr_input);
	RWCString MQUtility::SendMessageToQueueNew0420FromSAF(RWCString incoming, char *send_queue_input, char *queue_mgr_input, 
													  char *recv_queue);
	void MQUtility::SendMessageToQueueNewPagosPlataTransfer(RWCString incoming, char *send_queue_input, char *queue_mgr_input);
	RWCString MQUtility::SendMessageToQueueNewPagosTransfer(RWCString incoming, char *send_queue_input, char *queue_mgr_input,char *recv_queue_input,char *sql_queue_input);
	
	RWCString MQUtility::GetMsgFromRecvQueueHeaderWithTimeOut(  char *send_queue,char *queue_mgr, int time_out);
	RWCString MQUtility::SetIso8583B24FromBtransFormatReplica1C( RWCString incoming, char *application, char *header_msg ,int debug);
	RWCString MQUtility::SetBtransFormatFromB24Delimiter1C(RWCString incoming , char *msg_type, 
										          int debug, RWCString application);

	RWCString MQUtility::SendMessageToQueueNew0420FromSAFWithTimeOut(RWCString incoming, char *send_queue_input, char *queue_mgr_input, 
													  char *recv_queue, int time_out);
	RWCString MQUtility::SetIso8583B24Msg0210FromIso8583B24Msg0200ForScriptSimB24(RWCString incoming , char *msg_type, 
										          int debug, RWCString application,RWCString answer);
	
	RWCString MQUtility::SetIso8583B24FromTransPlata0420ForSAFVen(RWCString incoming , char *msg_type, 
										          int debug, RWCString application, RWCString field_125 );
	RWCString MQUtility::GetIso8583FieldB24ForVisaNet(RWCString incoming , char *msg_type, 
										          int debug, RWCString application,int input_field);
	RWCString MQUtility::SendMessageToQueueNewPagosPlataOnlyPagos0420New(RWCString incoming, char *send_queue_input, char *queue_mgr_input,char *recv_queue_string, char *sql_queue_string);
	
	RWCString MQUtility::SetTcpHeaderEPS( RWCString incoming_tcp_header ) ;
private:
	...
	RWCString responde_id_response          ;
	RWCString responde_code					;
