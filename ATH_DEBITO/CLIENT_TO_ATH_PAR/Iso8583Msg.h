...
public:
	...
	void ReturnDumpIsoGeneric( char *msg_type, int debugSB, RWCString peerAddress, int server_port);
  	int       SendMessageToQueueOld(RWCString incoming, char *recv_queue,char *queue_mgr);
	...
	RWCString Get_Msg_From_SendQueueOld( char *send_queue,char *queue_mgr);
	...
	int SendMessageToQueueSqlOld(RWCString incoming, char *sql_queue, char *queue_mgr);
